...
namespace BasaDann.Classes
{
    public class Requests
    {
        ...
        private static string _req_top_one = "SELECT id, name, company, content FROM request ORDER BY id DESC LIMIT 1";
        private static string _req_history = "select cusers_id, datetime, action from history where zid = ";
        private static string _meta = "<meta http-equiv='Content-Type' content='text/html;charset=UTF-8'>";
        private static string _req_comments = "SELECT * FROM comments where rid = ";

       // private static string _req_stat_company = "select * from request where company = 'УРАЛЬСКИЕ АВИАЛИНИИ ОАО АК' and pid=0"
        ...
        public static string Req_History
        {
            get { return _req_history; }
            // set { _req_top_one = value; }
        }
        ...
        public static string Req_Comments
        {
            get { return _req_comments; }
            // set { _req_top_one = value; }
        }
